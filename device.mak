PRODUCT_DEVICE := mirea_it1 # кодовое имя устройства

PRODUCT_NAME := lineage_it1 # кодовое имя продукта, указывается в
ro.product.name в файле build.prop скомпилированного образа

PRODUCT_BRAND := MIREA # бренд, указывается в ro.product.brand в файле
build.prop скомпилированного образа

PRODUCT_MODEL := MIREA it1 # модель, указывается в ro.product.model в
файле build.prop скомпилированного образа

PRODUCT_MANUFACTURER := MIREA # производитель, указывается в
ro.product.manufacturer в файле build.prop скомпилированного образа
...
ro.product.first_api_level=25 # переопределение уровня API Android,
с которым было выпущено устройство
